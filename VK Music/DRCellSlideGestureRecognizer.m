...
- (void)performAction {
	if (self.actionView.active) {
		...
		if (self.actionView.action.willTriggerBlock) self.actionView.action.willTriggerBlock(self.tableView, [self indexPath], self.cell);
		[self translateCellHorizontally:horizontalTranslation animatedlyWithDuration:ANIMATION_TIME damping:1 completion:^(BOOL finished) {
			...
			if (self.actionView.action.didTriggerBlock) self.actionView.action.didTriggerBlock(self.tableView, [self indexPath], self.cell);
		}];
